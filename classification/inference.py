...
class ClauseClassifier:
    ...
    def classify(self, text: str, threshold: float = 0.5, ner_entities: list | None = None) -> list[ClauseResult]:
        ...
        results = [
            r if r.present == (r.confidence >= threshold)
            else ClauseResult(r.clause_type, r.confidence >= threshold, r.confidence, r.evidence_spans)
            for r in results
        ]
...
def load_classifier(model_dir: str | None = None, labels_path: str | None = None,
    ...
    _CACHED_CLASSIFIER = ClauseClassifier(model_dir, labels, calibrators)
    return _CACHED_CLASSIFIER


def classify_clauses(text: str, threshold: float = 0.5, ner_entities: list | None = None,
